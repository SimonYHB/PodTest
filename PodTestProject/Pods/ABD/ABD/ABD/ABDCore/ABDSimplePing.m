...
- (NSData *)pingPacketWithType:(uint8_t)type payload:(NSData *)payload requiresChecksum:(BOOL)requiresChecksum {
    ...
    icmpPtr->code = 0;
    icmpPtr->checksum = 0;
    icmpPtr->identifier     = OSSwapHostToBigInt16(self.identifier);//! OCLINT
    icmpPtr->sequenceNumber = OSSwapHostToBigInt16(self.nextSequenceNumber);//! OCLINT
    ...
    if (icmpHeaderOffset != NSNotFound) {
        ...
        if (receivedChecksum == calculatedChecksum) {
            if ( (icmpPtr->type == ICMPv4TypeEchoReply) && (icmpPtr->code == 0) ) {
                if ( OSSwapBigToHostInt16(icmpPtr->identifier) == self.identifier ) {//! OCLINT
                    uint16_t    sequenceNumber;
                    
                    sequenceNumber = OSSwapBigToHostInt16(icmpPtr->sequenceNumber);//! OCLINT
...
static void SocketReadCallback(CFSocketRef s, CFSocketCallBackType type, CFDataRef address, const void *data, void *info) {
    ...
    obj = (__bridge ABDSimplePing *) info;
    assert([obj isKindOfClass:[ABDSimplePing class]]);
